...
const TIMELINE_SCALING_FACTOR = 45; // Increased from 15 to 45 (3x more stretching)

const Timeline = memo(({ className = "", height = 60 }: TimelineProps) => {
  ...
  useEffect(() => {
    ...
    if (Math.abs(x) < 10 && Math.abs(zoom - 1) < 0.1) {
      // Center the timeline view around position 30 for better initial view
      const initialX = -(30 * TIMELINE_SCALING_FACTOR) + (window.innerWidth / 2);
  ...
  const calculateVisibleRange = () => {
    ...
    // Adjust the calculation to account for the scaling factor
    const start = Math.floor(-x / (zoom * TIMELINE_SCALING_FACTOR));
    const end = start + Math.floor(containerWidth / (zoom * TIMELINE_SCALING_FACTOR));
  ...
  const generateTicks = () => {
    ...
    else if (zoom > 1.5) tickSpacing = 10;
    ...
    const startTick = Math.floor(constrainedStart / tickSpacing) * tickSpacing;
    const ticks = [];
    ...
    for (let pos = startTick; pos <= Math.min(end, 80); pos += tickSpacing) {
      ...
      const viewportPos = (pos * TIMELINE_SCALING_FACTOR * zoom) + x;
      ...
      ticks.push(
        <div 
          key={`tick-${pos}`}
          className={`absolute bottom-0 ${isMajorTick ? 'h-6 border-l-2' : 'h-5 border-l'} border-gray-500`}
          style={{ 
            left: `${viewportPos}px`,
    ...
    for (let pos = Math.max(0, Math.floor(start / 5) * 5); pos <= Math.min(end, 80); pos += 5) {
    ...
    for (let pos = Math.max(0, Math.floor(start)); pos <= Math.min(end, 80); pos += minorTickSpacing) {
  ...
  const generateNodeMarkers = () => {
    ...
    return nodes.map(node => {
      // Use the timelinePosition if available, otherwise use the current x position
      let nodePosition = node.data?.timelinePosition !== undefined 
        ? node.data.timelinePosition as number
        : node.position.x;
      ...
      const xPos = (nodePosition * TIMELINE_SCALING_FACTOR * zoom) + x;
    ...
    >
      <div className="relative w-full h-full">
        <div className="absolute top-2 left-4 text-sm font-medium text-gray-700">
          Timeline (0-80) | Zoom: {zoom.toFixed(1)}x
        </div>
